...
<body>
	<div class="container">
		...
		<form id="signupForm" class="form-signin" action="/WebClass/signup"
			...
			<input type="email" name="id" value="<%=user.getId()%>" id="inputEmail" class="form-control" placeholder="Email address" required autofocus>

			<label for="inputPassword" class="sr-only">Password</label> 
			<input type="password" name="pwd" id="inputPassword" value="<%=request.getAttribute("pwd")%>" class="form-control" placeholder="Password" required> 
			<label for="inputName" class="sr-only">Name</label> 
			<input type="text" name="name" value="<%=user.getName()%>" id="inputName" class="form-control" placeholder="Name" required> 
			<label for="inputNickName" class="sr-only">Nick Name</label> 
			<input type="text" name="nickname" id="inputNickName" value="<%=user.getNickname()%>" class="form-control" placeholder="Nickname" required> 
			<br>
	...
	<script>
			<%-- 로그인이 실패한 경우 처리 추가 --%>
			<%if ("error".equals(request.getAttribute("msg"))) {%>		var myModal = $('#myModal');
